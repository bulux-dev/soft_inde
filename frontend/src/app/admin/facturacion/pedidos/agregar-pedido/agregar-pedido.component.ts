...
export class AgregarPedidoComponent {
  ...
  async getDocumento() {
    this.pedidoForm.controls['documento_id'].setValue(this.documento_id);
    ...
    if (documento.code) {
      ...
      this.pedidos_detalles = JSON.parse(d) || [];
    }
  }
  ...
  async postPedido() {
    ...
    let pedido = await this.pedidos_service.postPedido(this.pedidoForm.value);
    if (pedido.code) {
  ...
  detalleLocal() {
    localStorage.setItem(`save_pedidos_${this.documento_id}`, JSON.stringify(this.pedidos_detalles));
  }

  updateCarrito() {
    this.pedidos_detalles = JSON.parse(localStorage.getItem(`carrito${this.documento_id}`) || '[]');
  }
