...
@Component({
  selector: 'app-agregar-nd_proveedor',
  standalone: false,
  templateUrl: './agregar-nd-proveedor.component.html',
  styleUrl: './agregar-nd-proveedor.component.css'
})
export class AgregarNDProveedorComponent {
  ...
  nd_proveedorForm: FormGroup = new FormGroup({
  ...
  async getCompras() {
    let compras = await this.compras_service.getAllComprasSaldos({
      proveedor_id: this.nd_proveedorForm.controls['proveedor_id'].value
    });
  ...
  async getDocumento() {
    this.nd_proveedorForm.controls['documento_id'].setValue(this.documento_id);
  ...
  async getMonedas() {
    ...
    if (monedas.code) {
      ...
      this.nd_proveedorForm.controls['moneda_id'].setValue([this.monedas[0]]);
    }
  }

  async getBodegasBySucursal(id: any) {
    this.nd_proveedorForm.controls['bodega_id'].setValue(null);
  ...
  async setDocumento() {
    this.nd_proveedorForm.controls['sucursal_id'].setValue([this.documento.sucursal]);

    await this.getBodegasBySucursal(this.documento.sucursal_id);

    if (this.documento.bodega) {
      this.nd_proveedorForm.controls['bodega_id'].setValue([this.documento.bodega]);
    } else {
      this.nd_proveedorForm.controls['bodega_id'].setValue(null);
    }
  ...
  addProveedor(i: any) {
    this.nd_proveedorForm.controls['proveedor_id'].setValue(i.id);
  ...
  async getInfoNit() {
    ...
    if (proveedor.data.length > 0) {
      ...
      this.nd_proveedorForm.controls['proveedor_id'].setValue(proveedor.data[0].id);
    } else {
      ...
      if (info.code && info.data.RESPONSE[0].NOMBRE) {
        ...
        if (proveedor) {
          ...
          this.nd_proveedorForm.controls['proveedor_id'].setValue(proveedor.data.id);
        }
  ...
  async postNDProveedor() {

    $('#btn_guardar').prop('disabled', true);
    this.ngxService.start();

    if (this.nd_proveedorForm.controls['bodega_id'].value && this.nd_proveedorForm.controls['bodega_id'].value.length > 0) {
      this.nd_proveedorForm.controls['bodega_id'].setValue(this.nd_proveedorForm.controls['bodega_id'].value[0].id);
    }
    if (this.nd_proveedorForm.controls['sucursal_id'].value && this.nd_proveedorForm.controls['sucursal_id'].value.length > 0) {
      this.nd_proveedorForm.controls['sucursal_id'].setValue(this.nd_proveedorForm.controls['sucursal_id'].value[0].id);
    }
    if (this.nd_proveedorForm.controls['moneda_id'].value && this.nd_proveedorForm.controls['moneda_id'].value.length > 0) {
      this.nd_proveedorForm.controls['moneda_id'].setValue(this.nd_proveedorForm.controls['moneda_id'].value[0].id);
    }

    this.nd_proveedorForm.controls['documento_id'].setValue(this.documento.id);
    this.nd_proveedorForm.controls['saldos'].setValue(this.saldos);

    let nd_proveedor = await this.nd_proveedores_service.postNDProveedor(this.nd_proveedorForm.value);
  ...
  getTotal() {
    let total = parseFloat(this.nd_proveedorForm.value.total);
    return total;
  }
