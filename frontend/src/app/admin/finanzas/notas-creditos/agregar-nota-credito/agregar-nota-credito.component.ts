...
@Component({
  selector: 'app-agregar-nota_credito',
  standalone: false,
  templateUrl: './agregar-nota-credito.component.html',
  styleUrl: './agregar-nota-credito.component.css'
})
export class AgregarNotaCreditoComponent {
  ...
  nota_creditoForm: FormGroup = new FormGroup({
  ...
  async getVentas() {
    let ventas = await this.ventas_service.getAllVentasSaldos({
      cliente_id: this.nota_creditoForm.controls['cliente_id'].value,
    });
  ...
  async getDocumento() {
    this.nota_creditoForm.controls['documento_id'].setValue(this.documento_id);
  ...
  async getMonedas() {
    ...
    if (monedas.code) {
      ...
      this.nota_creditoForm.controls['moneda_id'].setValue([this.monedas[0]]);
    }
  }

  async getBodegasBySucursal(id: any) {
    this.nota_creditoForm.controls['bodega_id'].setValue(null);
  ...
  async setDocumento() {
    this.nota_creditoForm.controls['sucursal_id'].setValue([this.documento.sucursal]);

    await this.getBodegasBySucursal(this.documento.sucursal_id);

    if (this.documento.bodega) {
      this.nota_creditoForm.controls['bodega_id'].setValue([this.documento.bodega]);
    } else {
      this.nota_creditoForm.controls['bodega_id'].setValue(null);
    }
  ...
  addCliente(i: any) {
    this.nota_creditoForm.controls['cliente_id'].setValue(i.id);
  ...
  async getInfoNit() {
    ...
    if (cliente.data.length > 0) {
      ...
      this.nota_creditoForm.controls['cliente_id'].setValue(cliente.data[0].id);
    } else {
      ...
      if (info.code && info.data.RESPONSE[0].NOMBRE) {
        ...
        if (cliente) {
          ...
          this.nota_creditoForm.controls['cliente_id'].setValue(cliente.data.id);
        }
  ...
  async postNotaCredito() {

    $('#btn_guardar').prop('disabled', true);
    this.ngxService.start();

    if (this.nota_creditoForm.controls['bodega_id'].value && this.nota_creditoForm.controls['bodega_id'].value.length > 0) {
      this.nota_creditoForm.controls['bodega_id'].setValue(this.nota_creditoForm.controls['bodega_id'].value[0].id);
    }
    if (this.nota_creditoForm.controls['sucursal_id'].value && this.nota_creditoForm.controls['sucursal_id'].value.length > 0) {
      this.nota_creditoForm.controls['sucursal_id'].setValue(this.nota_creditoForm.controls['sucursal_id'].value[0].id);
    }
    if (this.nota_creditoForm.controls['moneda_id'].value && this.nota_creditoForm.controls['moneda_id'].value.length > 0) {
      this.nota_creditoForm.controls['moneda_id'].setValue(this.nota_creditoForm.controls['moneda_id'].value[0].id);
    }

    this.nota_creditoForm.controls['nombre'].setValue(this.clienteForm.value.nombre);
    this.nota_creditoForm.controls['documento_id'].setValue(this.documento.id);
    this.nota_creditoForm.controls['saldos'].setValue(this.saldos);
    this.nota_creditoForm.controls['recibos_bancos'].setValue(this.recibos_bancos);
    this.nota_creditoForm.controls['cuenta_bancaria_id'].setValue(this.nota_creditoForm.controls['cuenta_bancaria_id'].value[0].id);

    let nota_credito = await this.notas_creditos_service.postNotaCredito(this.nota_creditoForm.value);
  ...
  getTotal() {
    let total = parseFloat(this.nota_creditoForm.value.total);
    return total;
  }
