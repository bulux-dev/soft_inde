...
import NotaCredito from './notas_creditos.model.js';
...
let getAllNotasCreditos = async (req, res) => {
  try {
    ...
    await resp.success({ mensaje: 'Notas Creditos encontradas', data: notas_creditos }, req, res, 'NotaCredito');
  } catch (err) {
    await resp.error(err, req, res);
  }
}

let getNotaCreditoDoc = async (req, res) => {
  try {
    let token = req.query.token;
    let data = jwt.decode(token, key);
    let empresa = await Empresa.findOne({ where: { id: data.data.empresa_id } });
    let nota_credito = await NotaCredito.findOne({
      where: { id: req.params.nota_credito_id },
      include: [
        {
          model: Cliente, as: 'cliente',
        },
    ...
    let file = fs.readFileSync(`./templates/nota_credito.html`, 'utf8');

    file = file.replace('___nota_credito', JSON.stringify(nota_credito));
...
let getOneNotaCredito = async (req, res) => {
  try {
    let nota_credito = await NotaCredito.findOne({ where: { id: req.params.id } });
    await resp.success({ mensaje: 'Nota Credito encontrada', data: nota_credito }, req, res, 'NotaCredito');
  } catch (err) {
    await resp.error(err, req, res);
  }
}

let createNotaCredito = async (req, res) => {
  try {
    ...
    let nota_credito = await NotaCredito.create(req.body);
    nota_credito.id = nota_credito.null;
    if (nota_credito) {
      ...
      if (saldos.length > 0) {
        ...
        saldos.forEach(async p => {
          ...
          p.nota_credito_id = nota_credito.id;
      ...
      if (recibos_bancos.length > 0) {
        for (let rb = 0; rb < recibos_bancos.length; rb++) {
          await Recibo.update({ nota_credito_id: nota_credito.id }, { where: { id: recibos_bancos[rb].recibo_id } });
        }
      }
    ...
    await resp.success({ mensaje: 'Nota Credito agregada', data: nota_credito }, req, res, 'NotaCredito');
  } catch (err) {
    await resp.error(err, req, res);
  }
}

let updateNotaCredito = async (req, res) => {
  try {
    let nota_credito = await NotaCredito.update(req.body, { where: { id: req.params.id } });
    await resp.success({ mensaje: 'Nota Credito actualizada', data: nota_credito }, req, res, 'NotaCredito');
  } catch (err) {
    await resp.error(err, req, res);
  }
}

let anularNotaCredito = async (req, res) => {
  try {
    let nota_credito = await NotaCredito.update(req.body, { where: { id: req.params.id } });
    ...
    let saldos = await Saldo.findAll({ where: { nota_credito_id: req.params.id } });
    saldos.forEach(async s => {
      ...
      await Saldo.create({
        ...
        venta_id: s.venta_id,
        nota_credito_id: s.nota_credito_id
      });
    })

    // Recibos Bancos
    let recibos_bancos = await Recibo.findAll({ where: { nota_credito_id: req.params.id } });
    for (let rb = 0; rb < recibos_bancos.length; rb++) {
      await Recibo.update({ nota_credito_id: null }, { where: { id: recibos_bancos[rb].id } });
    }

    await resp.success({ mensaje: 'Nota Credito anulada', data: nota_credito }, req, res, 'Nota Credito');
  } catch (err) {
    await resp.error(err, req, res);
  }
}

let deleteNotaCredito = async (req, res) => {
  try {
    let nota_credito = await NotaCredito.destroy({ where: { id: req.params.id } });
    await resp.success({ mensaje: 'Nota Credito eliminada', data: nota_credito }, req, res, 'NotaCredito');
  } catch (err) {
    await resp.error(err, req, res);
  }
}
